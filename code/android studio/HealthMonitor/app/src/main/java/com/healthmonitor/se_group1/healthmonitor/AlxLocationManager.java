...
public class AlxLocationManager implements GoogleApiClient.ConnectionCallbacks,GoogleApiClient.OnConnectionFailedListener,LocationListener {
    ...
    public static void onCreateGPS(final Application context){
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                Log.i("AlexLocation","use Android to get");
                Toast.makeText(manager.context,"please root", Toast.LENGTH_LONG).show();
                ...
                try {
                    ...
                    Log.i("AlexLocation","open locationtask wrong",e);
                }
                new AlxAsynTask<Void,Void,String>(){//在子线程中获取附近的基站和wifi信息
                    ...
                    protected void onPostExecute(String json) {
                        ...
                        Toast.makeText(context,"send to google api's json is；："+json, Toast.LENGTH_LONG).show();
                    }
                }.executeDependSDK();
            }
        },9000);
    ...
    public static class LocationTask extends TimerTask {
        ...
        @Override
        public void run() {
            Log.i("AlexLocation","location task use"+"manager is"+manager+"    destroy is"+AlxLocationService.isDestory);
    ...
    public static void pauseGPS(){
        ...
        try {
            ...
            Log.i("AlexLocation","stop GPS wrong",e);
        }
    }
    @Override
    public void onConnected(@Nullable Bundle bundle) {
        Log.i("AlexLocation","connect gps");
        ...
        if(!getCurrentLocation()) {//得到当前gps并记录
            ...
            LocationServices.FusedLocationApi.requestLocationUpdates(mGoogleApiClient, createFastLocationRequest(), this);//创建位置监听
            new TowerAndWiFiTask().executeDependSDK();
            Toast.makeText(manager.context,"keeping track", Toast.LENGTH_LONG).show();
            Log.i("AlexLocation","keep tracking");

        } else {//获取了最后一次硬件记录的经纬度，不进行追踪
            //在获取最近一次硬件经纬度成功之后要不要开启追踪模式，应该视项目的具体需求而定，如果对定位的准确度要求不是特别高的APP,那么拿到最近一次的定位就不用追踪了，减少耗电
            //对位置要求不高，要求省电的APP
            Toast.makeText(manager.context,"success", Toast.LENGTH_LONG).show();
            Log.i("AlexLocation","success");
    ...
    class TowerAndWiFiTask extends AlxAsynTask<Void,Void,String>{
        ...
        protected String doInBackground(Void... params) {
            ...
            if(geoLocationAPI ==null){
                Log.i("AlexLocation","false");
                return "{}";
            }
            getWifiInfo(context, geoLocationAPI);
            String json = geoLocationAPI.toJson();//这里使用gson.toJson()会被混淆，推荐使用手动拼json
            Log.i("AlexLocation","send goggle'json is"+json);
            return json;
        }
        ...
        protected void onPostExecute(String json) {
            ...
            Toast.makeText(context,"send google api's json is；："+json, Toast.LENGTH_LONG).show();
        }
    }
    ...
    public boolean getCurrentLocation(){
        ...
        Log.i("AlexLocation","last Location gps is=="+mLastLocation);
        ...
        Log.i("AlexLocation","last Location accuracy is"+last_accuracy);
        String provider = mLastLocation.getProvider();//传感器
        float bearing = mLastLocation.getBearing();
        float speed = mLastLocation.getSpeed();//速度
        if(isDebugging)
            Toast.makeText(context,"get last location  "+"latitude"+latitude+"  longitude"+longitude+ "accuracy"+last_accuracy, Toast.LENGTH_LONG).show();
        Log.i("AlexLocation","get last location，latitude=="+latitude+"  longitude"+longitude+"  altitude"+altitude+"   sensor"+provider+"   speed"+speed+ "accuracy"+last_accuracy);
        if(last_accuracy < MAX_deviation){
            recordLocation(context,latitude,longitude,last_accuracy);
        }else {
            Log.i("AlexLocation","accuracy low");
        }
    ...
    public void onLocationChanged(Location location) {
        if(currentStatus == STATUS.LOW_POWER) Log.i("Alex","power lower");
        Log.i("AlexLocation","change"+location);
        if(location==null)return;
        if(isDebugging) Toast.makeText(context,"get new GPS    "+location.toString()+" accuracy"+location.getAccuracy(), Toast.LENGTH_LONG).show();
    ...
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy){
        ...
        editor.apply();
        Log.i("AlexLocation","latitude="+latitude+"   longitude="+longitude+"   accuracy="+accuracy);
        ...
        if(myLocationStatic.updateTime == 0 || System.currentTimeMillis() - myLocationStatic.updateTime > SLOW_INTERVAL || accuracy <= myLocationStatic.accuracy) {
            ...
            if(isDebugging)
                Toast.makeText(context,"latitude result="+latitude+"   longitude="+longitude+"   accuracy="+accuracy, Toast.LENGTH_LONG).show();
        }else {
            Log.i("AlexLocation","accuracy low");
        }
    }
    ...
    public static double[] getOldLocation(Context context){
        ...
        Log.i("AlexLocation","accuracy"+accuracy);
    ...
    public static GeoLocationAPI getCellInfo(Context context){
        ...
        */
        ...
        Log.i("AlexLocation","information"+operator);
        if(operator==null || operator.length()<5){
            Log.i("AlexLocation","no sim card");
            return cellInfo;
        }
        ...
        if(Build.VERSION.SDK_INT<17) {//低版的android系统使用getNeighboringCellInfo方法
            ...
            if(infos==null){
                Log.i("AlexLocation","unsport");
                return cellInfo;
            }
            if(infos.size()==0)return cellInfo;//附近没有基站
            towers = new ArrayList<>(infos.size());
            StringBuffer sb = new StringBuffer("base station : " + infos.size() + "\n");
            ...
            Log.i("AlexLocation","information"+sb);
            ...
            if(infos!=null) {
                ...
                for (CellInfo i : infos) { // 根据邻区总数进行循环
                    Log.i("AlexLocation", "base station" + i.toString());//这里如果出现很多cid
                    GoogleCellTower tower = new GoogleCellTower();
                ...
                StringBuffer sb = new StringBuffer("base station number: " + infos2.size() + "\n");
                ...
                Log.i("AlexLocation","base station"+sb);
            }
        }
        cellInfo.cellTowers = towers;
        return cellInfo;
    ...
    public static boolean isWifiEnvironment(Context context){
        ...
        if(networkInfo==null){
            Log.i("AlexLocation","no Internet");
            return false;
        }
        int netType = networkInfo.getType();
        switch (netType){
            case ConnectivityManager.TYPE_WIFI:
                Log.i("AlexLocation","IP location");
                return true;
            case ConnectivityManager.TYPE_VPN://这个基本没用
                Log.i("AlexLocation","VPN");
                break;
            case ConnectivityManager.TYPE_MOBILE:
                Log.i("AlexLocation","cannot");
                int subType = networkInfo.getSubtype();
                Log.i("AlexLocation","sub"+subType+"  "+networkInfo.getSubtypeName());//能判断是2g/3g/4g网络
                break;
            default:
                Log.i("AlexLocation","unknow");
                break;
    ...
    public static boolean considerIP(Context context){
        ...
        if(Build.VERSION.SDK_INT<21) {//旧版本安卓获取网络状态
            ...
            for(NetworkInfo i:networkInfos){
                if(i==null)continue;
                Log.i("AlexLocation","now "+i.getTypeName()+i.getType()+"   "+i.getSubtypeName());//WIFI,VPN,MOBILE+LTE
                if(i.getType()== ConnectivityManager.TYPE_VPN){
                    Log.i("AlexLocation","cannot IP");
                    considerIP = false;
                    break;
                }
            }
            ...
            for(Network n:networks){
                ...
                Log.i("AlexLocation","now"+networkInfo.getTypeName()+networkInfo.getType()+"   "+networkInfo.getSubtypeName());//WIFI,VPN,MOBILE+LTE
    ...
    public static String determine2g3g4g(Context context){
        ...
        if(Build.VERSION.SDK_INT<21) {//旧版本安卓获取网络状态
            ...
            for(NetworkInfo i:networkInfos){
                if(i==null)continue;
                Log.i("AlexLocation","FINDING"+i.getTypeName()+i.getType()+"   "+i.getSubtypeName());//WIFI,VPN,MOBILE+LTE
                if(i.getType()!= ConnectivityManager.TYPE_MOBILE)continue;//只看流量
                else Log.i("AlexLocation","");
            ...
            for(Network n:networks){
                ...
                NetworkInfo networkInfo = connectivityManager.getNetworkInfo(n);
                if(networkInfo==null)continue;
                Log.i("AlexLocation",""+networkInfo.getTypeName()+networkInfo.getType()+"   "+networkInfo.getSubtypeName());//WIFI,VPN,MOBILE+LTE
    ...
    public static GeoLocationAPI getWifiInfo(Context context, GeoLocationAPI geoLocationAPI){
        ...
        if(lsScanResult == null){
            Log.i("AlexLocation","FILE");
            return geoLocationAPI;
        }
        for (ScanResult result : lsScanResult) {
            Log.i("AlexLocation","find wifi::"+result.SSID+"  mac "+result.BSSID+"   power"+result.level);
    ...
    public void sendJsonByPost(String json, String url){
        ...
        HttpRequest.post(url, params, new BaseHttpRequestCallback<String>(){
            ...
            protected void onSuccess(String s) {
                ...
                String result = s;
                Log.i("AlexLocation","succcess"+s);
                if(isDebugging) Toast.makeText(context,"return json is"+result, Toast.LENGTH_LONG).show();
                if(result==null || result.length()<10 || !result.startsWith("{")){
                    Log.i("AlexLocation","wrong"+result);
                }
                ...
                try {
                    ...
                    if(location==null){
                        Log.i("AlexLocation","wrong");
                        return;
                    }
                    double latitude = location.getDouble("lat");
                    double longitute = location.getDouble("lng");
                    double google_accuracy = returnJson.getDouble("accuracy");
                    Log.i("AlexLocation","return"+latitude+"  :  "+longitute+"  accuracy"+google_accuracy);
                    if(isDebugging)
                        Toast.makeText(context,"return"+latitude+"  :  "+longitute+"  accuracy"+google_accuracy, Toast.LENGTH_LONG).show();
                    recordLocation(context,latitude,longitute,(float) google_accuracy);//当没有从GPS获取经纬度成功，或者GPS的获取经纬度精确度不高，则使用基站和wifi的结果
                } catch (JSONException e) {
                    Log.i("AlexLocation","unknow",e);
                }
            ...
            public void onFailure(int errorCode, String msg) {
                ...
                Log.i("AlexLocation","fail"+msg+"   "+errorCode);
                if(errorCode==0) Log.i("AlexLocation","cannot");
                if(isDebugging) Toast.makeText(context,"wrong", Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void checkNetCardState(WifiManager mWifiManager) {
        if (mWifiManager.getWifiState() == 0) {
            Log.i("AlexLocation", "closeing");
        } else if (mWifiManager.getWifiState() == 1) {
            Log.i("AlexLocation", "close");
        } else if (mWifiManager.getWifiState() == 2) {
            Log.i("AlexLocation", "opening");
    ...
    public static double getGPSDistance(double lat_a, double lng_a, double lat_b, double lng_b) {
        ...
        double distance = (2 * EARTH_RADIUS * Math.asin(Math.sqrt(2 - 2 * Math.cos(x1)
                ...
                * Math.sin(x2)) / 2));
        Log.i("AlexLocation","distance"+distance);
        if(isDebugging && manager!=null)
            Toast.makeText(manager.context,"cahnge"+distance+"meter", Toast.LENGTH_LONG).show();
        return distance;
    }
